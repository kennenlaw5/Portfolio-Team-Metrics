...
function grab() { // first sheet -- return an array with data grabbed
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    sorted[i][2] = arr[i][7];         // home #
    sorted[i][3] = arr[i][8];         // work #
    sorted[i][4] = arr[i][9];         // email
    sorted[i][5] = arr[i][10];        // account #
    sorted[i][6] = arr[i][13];        // maturity date
    sorted[i][7] = arr[i][27];        // vin
    sorted[i][8] = arr[i][28];        // year
    sorted[i][9] = arr[i][29];        // make/model
